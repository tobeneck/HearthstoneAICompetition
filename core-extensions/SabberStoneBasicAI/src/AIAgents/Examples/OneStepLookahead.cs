...
{
	// Implemented by Tom Heimbrodt and winner of the 2019 Hearthstone AI Competition's Premade Deck Playing Track. Altered to only look ahead one step foor perfornamce reasons.
	class CustomScore2 : Score.Score
	{
	...
	class OneStepLookahead : AbstractAgent
	{
		...
		public override PlayerTask GetMove(POGame game)
		{
			...
			{
				List<int> mulligan = new CustomScore2().MulliganRule().Invoke(player.Choice.Choices.Select(p => game.getGame().IdEntityDic[p]).ToList());
			...
			var returnValue = validOpts.Any() ?
				validOpts.Select(x => score(x, player.PlayerId, (optcount >= 5) ? ((optcount >= 25) ? 1 : 1) : 1)).OrderBy(x => x.Value).Last().Key :
		...
		private static int Score(POGame state, int playerId)
		{
			...
			return new CustomScore2 { Controller = p }.Rate();
		}
